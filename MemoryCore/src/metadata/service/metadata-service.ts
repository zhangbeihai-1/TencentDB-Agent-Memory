...
export class MetadataService {
  ...
  async deleteUsersForCaller(userIds: string[], ctx: V3AuthContext): Promise<BatchDeleteResult> {
    ...
    for (const userId of userIds) {
      agentIds.push(...(await this.collectAgentIdsOfOwner(userId)));
    }
  ...
  async deleteTeams(teamIds: string[]): Promise<BatchDeleteResult> {
    ...
    for (const teamId of teamIds) {
      agentIds.push(...(await this.collectAgentIdsByTeam(teamId)));
    }
    if (agentIds.length > 0) await this.deleteAgents(agentIds);
  ...
  private async collectAgentIdsOfOwner(userId: string, teamId?: string): Promise<string[]> {
    ...
    for (;;) {
      const page = teamId
        ? await this.store.listAgentsByTeam(
          teamId,
          { limit: AGENT_CASCADE_PAGE, offset },
  ...
  private async collectAgentIdsByTeam(teamId: string): Promise<string[]> {
    const ids: string[] = [];
    let offset = 0;
    for (;;) {
