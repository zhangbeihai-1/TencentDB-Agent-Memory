...
export class SqliteMetadataStore implements IMetadataStore {
  ...
  deleteTeams(teamIds: string[]): BatchDeleteResult {
    if (teamIds.length === 0) return { deleted_ids: [], failed: [] };
    ...
    if (result.deleted_ids.length > 0) {
      const deletedPh = result.deleted_ids.map(() => "?").join(",");
      this.run(`DELETE FROM meta_team_members WHERE team_id IN (${deletedPh})`, ...result.deleted_ids);
      this.run(`DELETE FROM meta_tasks WHERE team_id IN (${deletedPh})`, ...result.deleted_ids);
      this.run(`DELETE FROM meta_assets WHERE team_id IN (${deletedPh})`, ...result.deleted_ids);
    }
    return result;
  }
