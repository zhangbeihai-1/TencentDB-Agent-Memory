...
export function runMetadataStoreContract(
  ...
  describe(`IMetadataStore contract: ${name}`, () => {
    ...
    describe("Delete Cascade", () => {
      ...
      it("deleteTeams uses the complete agent cascade", async () => {
        const owner = await store.createUser(uniqueUserInput());
        const team = await store.createTeam(teamInput(owner.user_id));
        const agent = await store.createAgent({
          team_id: team.team_id,
        ...
        expect(await store.getAssetById(memoryId)).toBeNull();
        expect((await store.listTaskAgents(task.task_id, P)).items).toHaveLength(0);
      });

      it("deleteTeams removes borrowed bindings before deleting team assets", async () => {
        const ownerA = await store.createUser(uniqueUserInput());
        const ownerB = await store.createUser(uniqueUserInput());
        const teamA = await store.createTeam(teamInput(ownerA.user_id, { name: "A" }));
        const teamB = await store.createTeam(teamInput(ownerB.user_id, { name: "B" }));
